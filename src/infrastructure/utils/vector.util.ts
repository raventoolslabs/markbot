...
export class VectorUtil {
  ...
  constructor() {
    this.textSplitter = new RecursiveCharacterTextSplitter({
      chunkSize: config.vector.maxChunkSize,
      chunkOverlap: config.vector.chunkOverlap,
      separators: ['\n## ', '\n### ', '\n#### ', '\n\n', '\n', ' ', ''],
    });
  }
  ...
  async processDocument(markdown: string): Promise<ProcessedDocument> {
    ...
    try {
      ...
      logger.info(
        `Generating embeddings for ${chunks.length} chunks in batches of ${config.vector.batchSize}...`,
        'VectorUtil',
      );

      // Process embeddings in batches for better performance
      for (let i = 0; i < chunks.length; i += config.vector.batchSize) {
        const batch = chunks.slice(i, i + config.vector.batchSize);
      ...
      logger.info(`Processed ${chunks.length} chunks and ${assets.length} assets in ${duration}ms`, 'VectorUtil');
  ...
  private async chunkMarkdown(markdown: string): Promise<ChunkDto[]> {
    ...
    for (let i = 0; i < textChunks.length; i++) {
      ...
      const mostSpecificSection =
        sectionInfo.path.length > 0 ? sectionInfo.path[sectionInfo.path.length - 1] : sectionInfo.title;
      ...
      while ((match = imageRefRegex.exec(chunk.content)) !== null) {
        ...
        if (imageData) {
          ...
          const existingAsset = assets.find(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (a) => a.asset_name === imageName && (a.metadata as any)?.chunk_index === index,
          );
  ...
  private async getExtractor() {
    if (!this.extractor) {
      logger.info(`Loading model ${config.vector.modelName}...`, 'VectorUtil');
      this.extractor = await pipeline('feature-extraction', config.vector.modelName);
