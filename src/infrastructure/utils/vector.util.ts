...
export class VectorUtil {
    ...
    /**
     * Processes a markdown string: chunks it and generates embeddings.
     */
    async processDocument(markdown: string): Promise<ChunkDto[]> {
        const chunks = this.chunkMarkdown(markdown);
        const extractor = await this.getExtractor();

        logger.info(`Generating embeddings for ${chunks.length} chunks...`, 'VectorUtil');
     ...
     */
    chunkMarkdown(markdown: string): ChunkDto[] {
    ...
    private async getExtractor() {
        if (!this.extractor) {
            logger.info(`Loading model ${this.modelName}...`, 'VectorUtil');
     ...
     */
    private processSection(chunks: ChunkDto[], text: string, sectionTitle: string, sectionPath: string[], startIndexOffset: number): number {
